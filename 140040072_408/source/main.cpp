...
using namespace std;
char* pi1; //playe1 score in character pointer
char* pi2;//player2 score in character pointer
char* p1; //player1 name in character pointer
char* p2; //player2 name in character pointer
char player1[20],player2[20]; //player1 player2 name
int score1=0, score2 = 0; //score of player1 and player in int
int c,x,y;
ALLEGRO_BITMAP *a;
ALLEGRO_BITMAP *b;
//header files
...
#include"dis_smile.h"
#include "convert.h" //to conver int to character pointer
#include "eqqq.h" // has equations stored in it
#include"country.h" // country and its capitals
#include "display.h" //
...
#include"maze.h"
#include"smiles.h"
#include"display_alphabet.h" //to display alphabet
#include"alphabet.h" // stored alphabets
void aftergame(char*eq1=" ",char*eq2=" ",char* eq3=" ")  //a function called after challenge is completed for instructions of next game
{
    ...
    ALLEGRO_COLOR electricblue = al_map_rgb(44,100,255); //defining a variable for color
    ...
    al_draw_text(font,al_map_rgb(44,255,0),200,100,ALLEGRO_ALIGN_CENTRE,p1); //dispaly name of player1
    al_draw_text(font,al_map_rgb(44,255,0),1100,100,ALLEGRO_ALIGN_CENTRE,p2); //dispaly name of player2
    ...
    al_flip_display();
    al_rest(2.0); // to rest the screen displayed for 2 secs
    return;
...
int main()
{
    ...
    gets(player2);
    p1= player1; //storing name of player1 in a character pointer
    p2= player2; //storing name of player1 in a character pointer

    ALLEGRO_DISPLAY *display; //create display screen

    al_init();
    al_init_font_addon(); //calling functions to add text
    al_init_ttf_addon();
    al_init_image_addon(); //calling functions to add image
    al_install_mouse(); //calling function to install mouse
    al_install_keyboard(); //calling function to install keyboard
    ALLEGRO_EVENT_QUEUE *event_queue=al_create_event_queue(); //crreating an event
    ALLEGRO_EVENT_QUEUE *event_queue1=al_create_event_queue();
    al_register_event_source(event_queue,al_get_mouse_event_source()); //registering an event
    ...
    al_clear_to_color(al_map_rgb(0,0,0)); //setting background to black
    //to draw line
    al_init_primitives_addon();
    ...
    al_flip_display();
    al_rest(1.0);
X: //label to go to main menu again
    //2nd screen
    ...
    {
        al_wait_for_event(event_queue,&events);
        x=events.mouse.x; //x and y coordinate of mouse click is obtained
        y=events.mouse.y;
    }
    if (y>325 && y<=550)  //checking the coordinates where the mouse is clicked
    ...
    {
        //displaying about the developer
        al_clear_to_color(al_map_rgb(0,0,0));
        ALLEGRO_FONT *fontx= al_load_font("RiseInside-Regular.ttf",36,NULL);
    ...
    else if (y>0 && y<=325)
    {
       ...
       //final comparing the score to display the winner
        if(score1>score2)
        {
